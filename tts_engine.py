...
def _resolve_checkpoint(checkpoint_ref: str) -> Path:
    """Resolve a local checkpoint path or download a gs:// object to /tmp.

    Cloud Run instances have a writable ephemeral /tmp filesystem. The model
    is downloaded once during process startup and reused by the loaded VITS
    model for the lifetime of that instance.
    """
    if not checkpoint_ref.startswith("gs://"):
        path = Path(checkpoint_ref)
        if not path.exists():
            raise FileNotFoundError(f"TTS checkpoint가 없습니다: {path}")
        return path

    gcs_path = checkpoint_ref[5:]
    if "/" not in gcs_path:
        raise ValueError(f"잘못된 GCS URI입니다: {checkpoint_ref}")
    ...
    local_path = Path(
        os.getenv("TTS_CHECKPOINT_CACHE_PATH", "/tmp/jeju_vits.pth")
    )
    local_path.parent.mkdir(parents=True, exist_ok=True)

    if local_path.exists() and local_path.stat().st_size > 0:
        print(f"TTS checkpoint 캐시 사용: {local_path}")
        return local_path

    print(f"TTS checkpoint GCS 다운로드: {checkpoint_ref} -> {local_path}")
    ...
    if not local_path.exists() or local_path.stat().st_size == 0:
        raise RuntimeError(f"GCS에서 TTS checkpoint 다운로드에 실패했습니다: {checkpoint_ref}")

    print(f"TTS checkpoint 다운로드 완료: {local_path.stat().st_size / 1024 / 1024:.1f} MB")
    return local_path
...
class JejuVITSEngine:
    ...
    def __init__(
        ...
        self.checkpoint_uri = checkpoint_path
        if not self.config_path.exists():
            raise FileNotFoundError(f"TTS config가 없습니다: {self.config_path}")

        # Supports both local paths and gs://bucket/object URIs.
